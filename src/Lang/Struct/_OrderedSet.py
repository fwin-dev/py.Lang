	...
	*** NOTE THAT SLICE NOTATION IS STILL BEING IMPLEMENTED ***
	...
	def __setitem__(self, indexOrSlice, value):
		if isinstance(indexOrSlice, slice):
			index_start = indexOrSlice.start if indexOrSlice.start != None else 0
			if index_start == indexOrSlice.stop:	# insertion mode - same as python standard library
				...
				links = self._iterLinks(indexOrSlice)
				if len([link for link in links]) != len(value):
